...
function showAfterPrintPdf()
{
    ...
    document.querySelector("#table_search_paginate").style.display = "inline-block";
...
function mainPage(mainMaleOrFemale = 'all',areaName = 'all',committees='all',headquarters='all')
{
...
$(".next_id_task").on("click",function(){
    ...
    $.ajax({
        ...
        success: function(response){
            $("#form_tasks")[0].reset();
            $("#id_show_task").val(`${response.id}`)
            $("#title_task").html(`${response.title} <a class="btn btn-success" href='${hrefAdd}'>+<a/>`);
            $("#id_task").val(response.id)
            $("#title_task_page").val(response.title)
            $("#date_task").val(response.date)
            $("#desc_task").val(response.descrption)
            $("#importance_task").val(response.importance)
...
$(".next_id_trans").on("click",function(){
    ...
    $.ajax({
        ...
        type: 'post',
        dataType:'json',
        data: {action:'show_next_trans',id:id_trans,id_user:id_user,type:type},
        success: function(response){
            $("#form_trans")[0].reset();
            $("#id_show_trans").val(`${response.id}`)
            $("#id_show_trans_hidden").val(`${response.id}`)
            
            $("#title_trans").html(`${response.applicant} <a class="btn btn-success" href='${hrefAdd}'>+<a/>`);
            $("#date").val(response.date_trans)
            $("#applicant").val(response.applicant)
            $("#phone_applicant").val(response.phone_applicant)
            $("#order_by").val(response.order_by)
            $("#phone_order_by").val(response.phone_order_by)
            $("#almueamala").val(response.almueamala)
            $("#details").val(response.details)
            $(".direct_to").each(function(key,value) {
                $(this).attr("selected",false);
                if($(this).val() == response.direct_to)
                {
            ...
            $(".importance").each(function(key,value) {
                $(this).attr("selected",false);
                if($(this).val() == response.importance)
                {
                    $(this).attr("selected",true)
                }
            ...
            $(".option_order").each(function(key,value) {
                ...
                if($(this).val() == response.option_order)
                {
